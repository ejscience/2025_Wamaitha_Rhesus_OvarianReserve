...
nano.obj <- LoadNanostring(data.dir = "~/Desktop/CoSMX/ONPRC020", fov = "emb020")
...
ImageDimPlot(nano.obj, fov = "emb020", axes = TRUE, cols = "glasbey")
AllMarkers <- FindAllMarkers(nano.obj, only.pos = TRUE, logfc.threshold = 0)
write.csv(AllMarkers, "~/Desktop/emb020_DEGs.csv")
write.csv(nano.obj@active.ident, "~/Desktop/emb020_cell_type.txt")
AllMarkers <- FindAllMarkers(nano.obj, only.pos = TRUE, logfc.threshold = 0)
write.csv(AllMarkers, "~/Desktop/emb020_After_Annotation_DEGs.csv")
...
ImageDimPlot(Gonad_1, fov = "emb020", axes = TRUE, cols = "glasbey")
...
ImageDimPlot(Gonad_1, axes = TRUE, fov = "emb020", group.by = "predicted.celltype") + scale_fill_manual(values = batlowW)
ggsave("~/Desktop/Gonad_1_annotated.tiff", device = grDevices::tiff)

ImageFeaturePlot(Gonad_1, fov = "emb020",  features = c("NANOG", "KITLG", "KRT18", "KRT19", "NR2F2", "PDGFRA", "POU5F1", "PTGDS"), max.cutoff = "q95") + NoLegend()
ggsave("~/Desktop/CosMX_results_emb020/Gonad_1_genes.tiff", device = grDevices::tiff)

ImageFeaturePlot(Gonad_1, fov = "emb020",  features = c("MYL4", "MYL7", "OAS3", "RNF34", "CAV1", "MKI67"), max.cutoff = "q95") + NoLegend()
...
###ZOOM OVARY#####
basal.crop <- Crop(Gonad_1[["emb020"]], coords = "tissue", x = c(11000, 19000), y = c(110000, 123200))
...
ggsave("~/Desktop/Ovary_Zoom_Gonad_1.tiff", device = grDevices::tiff)
...
ggsave("~/Desktop/Genes_Granulosa_Gonad_1.tiff", device = grDevices::tiff)
...
ggsave("~/Desktop/Genes_Germline_Gonad_1.tiff", device = grDevices::tiff)
...
ggsave("~/Desktop/Genes_Stroma_Gonad_1.tiff", device = grDevices::tiff)
...
ImageDimPlot(Gonad_2, fov = "emb020", axes = TRUE, cols = "glasbey")
...
ImageDimPlot(Gonad_2, axes = TRUE, fov = "emb020", group.by = "predicted.celltype") + scale_fill_manual(values = batlowW)
ggsave("~/Desktop/Gonad_2_annotated.tiff", device = grDevices::tiff)

ImageFeaturePlot(Gonad_2, fov = "emb020",  features = c("NANOG", "KITLG", "KRT18", "KRT19", "NR2F2", "PDGFRA", "POU5F1", "PTGDS"), max.cutoff = "q95") + NoLegend()
ggsave("~/Desktop/CosMX_results_emb020/Gonad_2_genes.tiff", device = grDevices::tiff)

ImageFeaturePlot(Gonad_2, fov = "emb020",  features = c("MYL4", "MYL7", "OAS3", "RNF34", "CAV1", "MKI67"), max.cutoff = "q95") + NoLegend()
...
write.csv(AllMarkers, "~/Desktop/CosMX_results_emb020/After_annotation_Gonad_2_DEGs.csv")

###ZOOM OVARY#####
basal.crop <- Crop(Gonad_2[["emb020"]], coords = "tissue", x = c(12500, 21000), y = c(7000, 16000))
...
ggsave("~/Desktop/Ovary_Zoom_Gonad_2.tiff", device = grDevices::tiff)
...
ggsave("~/Desktop/Genes_Granulosa_Gonad_2.tiff", device = grDevices::tiff)
...
ggsave("~/Desktop/Genes_Germline_Gonad_2.tiff", device = grDevices::tiff)
...
ggsave("~/Desktop/Genes_Stroma_Gonad_2.tiff", device = grDevices::tiff)
