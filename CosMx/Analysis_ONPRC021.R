...
nano.obj <- LoadNanostring(data.dir = "~/Desktop/CoSMX/ONPRC021", fov = "emb021")
...
ImageDimPlot(nano.obj, fov = "emb021", axes = TRUE, cols = "glasbey")
AllMarkers <- FindAllMarkers(nano.obj, only.pos = TRUE, logfc.threshold = 0)
write.csv(AllMarkers, "~/Desktop/Nano_emb021_DEGs.csv")
...
ImageDimPlot(Gonad_1, axes = TRUE, fov = "emb021", cols = "glasbey")
ggsave("~/Desktop/CosMX_results_emb021/test.tiff", device = grDevices::tiff)
dev.off()
AllMarkers <- FindAllMarkers(Gonad_1, only.pos = TRUE, logfc.threshold = 0)
write.csv(AllMarkers, "~/Desktop/CosMX_results_emb021/Gonad_1_DEGs.csv")
...
ImageDimPlot(Gonad_1, axes = TRUE, fov = "emb021", cols = batlowW, group.by = "predicted.celltype")
...
ImageDimPlot(Gonad_1, axes = TRUE, fov = "emb021", group.by = "predicted.celltype") + scale_fill_manual(values = batlowW)
ggsave("~/Desktop/Gonad_1_annotated.tiff", device = grDevices::tiff)

###ZOOM OVARY#####
basal.crop <- Crop(Gonad_1[["emb021"]], coords = "tissue", x = c(900, 4600), y = c(107200, 113500))
...
ImageFeaturePlot(Gonad_1, fov = "emb021",  features = c("NANOG", "KITLG", "KRT18", "KRT19", "NR2F2", "PDGFRA", "POU5F1", "PTGDS"), max.cutoff = "q95") + NoLegend()
ggsave("~/Desktop/CosMX_results_emb021/Gonad_1_genes.tiff", device = grDevices::tiff)

ImageFeaturePlot(Gonad_1, fov = "emb021",  features = c("MYL4", "MYL7", "OAS3", "RNF34", "CAV1", "MKI67"), max.cutoff = "q95") + NoLegend()
ggsave("~/Desktop/Gonad_1_genes2.tiff", device = grDevices::tiff)

Idents(object = Gonad_1) <- "predicted.celltype"
AllMarkers <- FindAllMarkers(Gonad_1, only.pos = TRUE, logfc.threshold = 0)
write.csv(AllMarkers, "~/Desktop/CosMX_results_emb021/After_annotation_Gonad_1_DEGs.csv")
...
ImageDimPlot(Gonad_2, fov = "emb021", axes = TRUE, cols = "glasbey")
ggsave("~/Desktop/CosMX_results_emb021/Gonad_2_Image_DimPlot.tiff", device = grDevices::tiff)
AllMarkers <- FindAllMarkers(Gonad_2, only.pos = TRUE, logfc.threshold = 0)
write.csv(AllMarkers, "~/Desktop/CosMX_results_emb021/Gonad_2_DEGs.csv")

write.csv(Gonad_2@active.ident, "~/Desktop/Gonad_2_cell_type.txt")
Gonad_2@meta.data[["predicted.celltype"]] <- Gonad_2_cell_type$V2

ImageDimPlot(Gonad_2, axes = TRUE, fov = "emb021", cols = "glasbey", group.by = "predicted.celltype")
ggsave("~/Desktop/CosMX_results_emb021/Gonad_2_annotated.tiff", device = grDevices::tiff)

ImageFeaturePlot(Gonad_2, fov = "emb021",  features = c("NANOG", "KITLG", "KRT18", "KRT19", "NR2F2", "PDGFRA", "POU5F1", "PTGDS"), max.cutoff = "q95") + NoLegend()
ggsave("~/Desktop/CosMX_results_emb021/Gonad_2_genes.tiff", device = grDevices::tiff)

Idents(object = Gonad_2) <- "predicted.celltype"
AllMarkers <- FindAllMarkers(Gonad_2, only.pos = TRUE, logfc.threshold = 0)
write.csv(AllMarkers, "~/Desktop/CosMX_results_emb021/After_annotation_Gonad_2_DEGs.csv")

###ZOOM OVARY#####
basal.crop <- Crop(Gonad_2[["emb021"]], coords = "tissue", x = c(11000, 15000), y = c(-1000, 3000))
...
ImageDimPlot(Gonad_3, fov = "emb021", axes = TRUE, cols = "glasbey")
ggsave("~/Desktop/CosMX_results_emb021/Gonad_3_Image_DimPlot.tiff", device = grDevices::tiff)
AllMarkers <- FindAllMarkers(Gonad_3, only.pos = TRUE, logfc.threshold = 0)
write.csv(AllMarkers, "~/Desktop/CosMX_results_emb021/Gonad_3_DEGs.csv")

write.csv(Gonad_3@active.ident, "~/Desktop/Gonad_3_cell_type.txt")
Gonad_3@meta.data[["predicted.celltype"]] <- Gonad_3_cell_type$V2

ImageDimPlot(Gonad_3, axes = TRUE, fov = "emb021", cols = "glasbey", group.by = "predicted.celltype")
ggsave("~/Desktop/CosMX_results_emb021/Gonad_3_annotated.tiff", device = grDevices::tiff)

ImageFeaturePlot(Gonad_3, fov = "emb021",  features = c("NANOG", "KITLG", "KRT18", "KRT19", "NR2F2", "PDGFRA", "POU5F1", "PTGDS"), max.cutoff = "q95") + NoLegend()
ggsave("~/Desktop/CosMX_results_emb021/Gonad_3_genes.tiff", device = grDevices::tiff)

Idents(object = Gonad_3) <- "predicted.celltype"
AllMarkers <- FindAllMarkers(Gonad_3, only.pos = TRUE, logfc.threshold = 0)
write.csv(AllMarkers, "~/Desktop/CosMX_results_emb021/After_annotation_Gonad_3_DEGs.csv")

###ZOOM OVARY#####
basal.crop <- Crop(Gonad_3[["emb021"]], coords = "tissue", x = c(14000, 18000), y = c(120500, 124000))
